...
for i in range(1, len(soup.root.contents), 2):
    ...
    for j in range(5, len(cyclone.contents), 2):
        ...
        c.depth.append(c.outerPressure[-1] - c.pressure[-1])
        c.velocity.append(float(cyclone.contents[j].velocity.text))
        #c.part.append(cyclone.contents[j].part.text)
        #if c.radiusMin[-1] > c.radiusMax[-1]:
        #    c.radiusMin[-1] /= 2
        #    if c.part[-1] == 'half':
        #c.area2.append(c.radiusMax[-1] * c.radiusMin[-1] * pi)
        #if c.part[-1] == 'half':
        #    c.area2[-1] /= 2
        #elif c.part[-1] == 'quoter':
        #    c.area2[-1] /= 4
...
for c in cyclones:
    ...
    if flag:
        ...
        flag = False
        ################################### immediate
        l = [abs((ts - fluidDates[id]).days * 24 + (ts - fluidDates[id]).seconds//3600) for ts in c.timestamp]
        min1 = min(l)
...
writer = open("velocity.csv", "wb")
entries = ['%s  \n' % ",".join((format(x, "10.3f") for x in i.velocity)) for i in cyclones]
for e in entries:
    writer.write(e)
writer.close()
...
saveToFile(cyclones, 'baltik_flood_immediate')
