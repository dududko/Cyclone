...
soup = BeautifulSoup(open('..\\test\\result_all_new\\baltik.xml'))  # get the parser for the xml file
...
for c in cyclones:
    ...
    wasAll = False
    for i in c.ids:
        if isOnBoltik(c.latitude[i], c.longitude[i], c.radiusMax[i]):
            if wasAll:
                elem[1] += 1
                wasAll = True
            else:
